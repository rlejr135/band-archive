...
public class BackgroundUploadPlugin extends Plugin {
  ...
  @PermissionCallback private void notificationPermissionResult(PluginCall call){call.resolve(new JSObject().put("granted",getPermissionState("notifications")==PermissionState.GRANTED));}
  ...
  @PluginMethod public void cancel(PluginCall call){String id=call.getString("id");UploadScheduler.cancel(getContext(),id);store.cancel(id);new Thread(()->UploadEngine.abort(getContext(),id)).start();UploadStore.Task t=store.get(id);if(t!=null)UploadEvents.emit(t);call.resolve();}
