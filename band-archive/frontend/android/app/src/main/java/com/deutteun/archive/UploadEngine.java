...
public final class UploadEngine {
  ...
  private static final ThreadLocal<String> LEASE_OWNER = new ThreadLocal<>();
  ...
  public static Outcome run(Context context, String id) {
    ...
    String owner = UUID.randomUUID().toString(); if (!store.acquire(id, owner)) return Outcome.SUCCESS; LEASE_OWNER.set(owner);
    try { try {
      File file = durableFile(task); verifyFile(file, task);
      task.state = "preparing"; update(store, task);
      Session session = openOrResume(task, store);
      JSONObject remote = get(task.api + "/uploads/multipart/" + session.id, session.capability);
      String remoteState = remote.optString("status");
      if ("completed".equals(remoteState)) { task.result = minimalResult(remote); task.progress = 100; task.state = queueState(remote); update(store, task); deleteSource(task); return "failed".equals(task.state) ? Outcome.FAILURE : Outcome.SUCCESS; }
      ...
      for (int part = 1; part <= parts; part++) if (!acked.contains(part)) uploadPart(task, store, file, session, part, partSize);
      task.state = "completing"; update(store, task);
      ...
      update(store, task); deleteSource(task); return Outcome.SUCCESS;
    } catch (TerminalException error) { return fail(store, task, error, false); }
      catch (Exception error) { return fail(store, task, error, true); }
    } finally { store.release(id, owner); LEASE_OWNER.remove(); }
  }

  public static void abort(Context context, String id) {
    UploadStore store = new UploadStore(context); UploadStore.Task task = store.get(id); if (task == null) return;
    ...
    task.state = "cancelled"; update(store, task); deleteSource(task);
  }

  private static Outcome fail(UploadStore store, UploadStore.Task task, Exception error, boolean retryable) {
    UploadStore.Task latest = store.get(task.id); if (latest != null && "cancelled".equals(latest.state)) return Outcome.SUCCESS;
    task.attempts++; task.error = userMessage(error);
    task.state = retryable && task.attempts < MAX_ATTEMPTS ? "retry_wait" : "failed";
    update(store, task); return "retry_wait".equals(task.state) ? Outcome.RETRY : Outcome.FAILURE;
  }
  private static void update(UploadStore s, UploadStore.Task t) { String owner=LEASE_OWNER.get(); if(owner!=null&&!s.renew(t.id,owner)) throw new IllegalStateException("업로드 실행 권한이 만료되었습니다."); s.update(t); UploadEvents.emit(t); }
  ...
  private static Session openOrResume(UploadStore.Task t, UploadStore s) throws Exception {
    ...
    JSONObject init = post(t.api + "/uploads/multipart/initiate", new JSONObject(t.target)
    ...
    t.session = init.getString("session_id"); t.capability = capability; update(s, t);
  ...
  private static void uploadPart(UploadStore.Task t, UploadStore s, File f, Session session, int number, long partSize) throws Exception {
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) try {
      JSONObject issued = post(t.api + "/uploads/multipart/" + session.id + "/parts", new JSONObject().put("part_number", number), session.capability);
      ...
      String etag = put(issued.getString("upload_url"), f, offset, length, t.mime, sent -> {
        ...
        if (progress != lastReported[0]) { lastReported[0] = progress; t.state = "uploading"; t.progress = progress; update(s, t); }
      });
      ...
      t.state = "uploading"; t.progress = (int) Math.min(99, ((offset + length) * 100) / t.bytes); update(s, t); return;
    } catch (HttpException e) {
      last = e; if (!e.retryable || attempt == MAX_ATTEMPTS - 1) break; retry(t, s, attempt);
    } catch (IOException e) {
      last = e; if (attempt == MAX_ATTEMPTS - 1) break; retry(t, s, attempt);
    }
  ...
  private static void retry(UploadStore.Task t, UploadStore s, int attempt) throws InterruptedException { t.state = "retry_wait"; update(s, t); Thread.sleep(UploadProtocol.retryDelayMs(attempt)); }
  private static JSONObject get(String url, String cap) throws Exception { HttpURLConnection c = connection(url, "GET", cap); return json(c); }
  private static JSONObject post(String url, JSONObject body, String cap) throws Exception {
    ...
    try (OutputStream out = c.getOutputStream()) { out.write(body.toString().getBytes(StandardCharsets.UTF_8)); } return json(c);
  }
  private static HttpURLConnection connection(String url, String method, String cap) throws Exception {
    ...
    if (cap != null) c.setRequestProperty("X-Upload-Capability", cap); return c;
  }
  private interface Progress { void onBytes(long sent); }
