...
const uploadSingle = async ({ file, songId, rehearsalId, onProgress, registerXhr }) => {
  const contentType = file.type || 'application/octet-stream';
  const presign = await requestJson(`${API_URL}/uploads/presign`, {
    method: 'POST', body: JSON.stringify({ filename: file.name, content_type: contentType, upload_type: 'media' }),
  });
  await putWithProgress(presign.upload_url, file, contentType, (loaded) => onProgress(loaded, file.size), registerXhr);
  return requestJson(`${API_URL}/uploads/complete/media`, {
    method: 'POST',
    body: JSON.stringify({ filename: presign.filename, original_filename: file.name, file_size: file.size, song_id: songId, rehearsal_id: rehearsalId || null }),
  });
};

const uploadMultipart = async ({ file, songId, rehearsalId, onProgress, registerXhr, setSessionId }) => {
  const initiated = await requestJson(`${API_URL}/uploads/multipart/initiate`, {
    method: 'POST',
    body: JSON.stringify({ filename: file.name, content_type: file.type || undefined, declared_bytes: file.size, song_id: songId, rehearsal_id: rehearsalId || null }),
  });
...
export const fetchMediaProcessing = async (mediaId) => normalizeMedia(await requestJson(`${API_URL}/media/${mediaId}/processing`));
export const retryMediaAudio = async (mediaId) => normalizeMedia(await requestJson(`${API_URL}/media/${mediaId}/retry-audio`, { method: 'POST', body: JSON.stringify({}) }));
